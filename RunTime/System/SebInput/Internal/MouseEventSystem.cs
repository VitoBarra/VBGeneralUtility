...
namespace SebInput.Internal
{
    ...
    public class MouseEventSystem : MonoBehaviour
    {
        ...
        public static void AddInteractionListener(MouseInteractionListener listener) =>
            GetOrCreateInstance().RegisterListener(listener);

        public static void RemoveInteractionListener(MouseInteractionListener listener) =>
            instance?.DeregisterListener(listener);
        ...
        void Awake()
        {
            ...
            {
                Debug.Log(
                    $"Duplicate Mouse Event System found ({instance.gameObject.name}). Deleting this instance ({gameObject.name}).");
                Destroy(gameObject);
            }
        }
        ...
        void Update()
        {
            ...
            if (Application.isEditor)
            {
                vid_mouseSmoothPos = Vector2.SmoothDamp(vid_mouseSmoothPos, Mouse.current.position.ReadValue(),
                    ref vid_mouseSmoothV, vid_mouseSmoothT);
            }
        ...
        void NotifyMouseEnter(Transform enter)
        {
            ...
            for (int i = listenersWithoutMouseOver.Count - 1; i >= 0; i--)
            {
                ...
                if (!Belongs(enter, listener)) continue;
